...
export type ZoomConnector = { createUniqueMeeting(input: { bookingId: string; startsAt: string; durationMinutes: number; idempotencyKey: string }): Promise<{ providerMeetingId: string; privateJoinMetadataCiphertext: string }>; retireMeeting?(providerMeetingId: string): Promise<void> };
