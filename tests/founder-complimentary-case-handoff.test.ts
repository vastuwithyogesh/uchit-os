...
function setup() {
  ...
  state.prospectiveProjects.push({ id: "prospective-test-only", organisationId, clientId: "UC-TEST-ONLY-COMPLIMENTARY", leadId: "lead-test-only", responseVersionId: "response-test-only", kind: "RESIDENTIAL", status: "COMMERCIAL_PENDING", serviceType: "EXISTING_SPACE", createdAt: "2026-08-13T00:00:00.000Z", recordVersion: 1 });
...
test("accepted internal complimentary proposal creates one canonical case/project/floor without payment or invoice", async () => {
  ...
  assert.equal(created.status, "CASE_CREATED"); assert.equal(created.serviceType, "EXISTING_SPACE"); assert.equal(state.vastuCases.length, 1); assert.equal(state.projects.length, 1); assert.equal(state.floorWorkspaces.length, 1);
