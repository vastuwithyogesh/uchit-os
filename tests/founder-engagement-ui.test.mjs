...
test("manual compose sheet renders both channels and claims PREPARED or OPENED only",()=>{assert.match(sheet,/WhatsApp.*NOT PREPARED/s);assert.match(sheet,/Email.*NOT PREPARED/s);assert.match(sheet,/Open WhatsApp/);assert.match(sheet,/Open Gmail draft/);assert.match(sheet,/Use default email app/);assert.match(sheet,/Yogesh must review and press Send manually/);assert.match(sheet,/Allow pop-ups and retry; no OPENED state was recorded/);});
