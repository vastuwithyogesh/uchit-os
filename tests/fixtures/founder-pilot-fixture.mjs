...
export function buildReleaseableFounderPilotFixture() {
  ...
  const verifiedDocument = (assetType, floorLabel) => owned({ id: `pilot-document-${assetType.toLowerCase()}`, caseId: pilotIds.caseId, caseRevisionNumber: 1, serviceType: "EXISTING_SPACE", ...(floorLabel ? { floorLabel } : {}), assetType, versionLabel: `Synthetic ${assetType.toLowerCase()} v1`, documentDate: "2026-08-11", isCurrent: true, evidenceRef: `case-file-pilot-${assetType.toLowerCase()}`, blocker: false, ownerRole: "CONSULTANT", ownerName: "Founder Pilot Operator", revisionStatus: "VERIFIED", idempotencyKey: `pilot-document-${assetType.toLowerCase()}`, version: 1, received: { actorId: pilotIds.founderId, actorName: "Founder Pilot Operator", actorRole: "SUPER_ADMIN", at: PILOT_TIME }, verified: { actorId: pilotIds.founderId, actorName: "Founder Pilot Operator", actorRole: "SUPER_ADMIN", at: PILOT_TIME }, updated: { actorId: pilotIds.founderId, actorName: "Founder Pilot Operator", actorRole: "SUPER_ADMIN", at: PILOT_TIME }, createdAt: PILOT_TIME });
  const pilotCaseDocuments = ["LOCATION_MAP", "PHOTO_VIDEO", "ENTRANCE_ACCESS", "CURRENT_USE", "STRUCTURE_SERVICES", "FURNITURE_EQUIPMENT", "CLIENT_PRIORITIES"].map((assetType) => verifiedDocument(assetType));
  pilotCaseDocuments.push(verifiedDocument("DIMENSIONED_PLAN", "Ground floor"));
  const state = {
    ...
    clientIntakeProfiles: [owned({ id: "pilot-intake", clientId: pilotIds.clientId, version: 1, propertyContext: { serviceInterest: "EXISTING_SPACE", propertyType: "Residential", propertyStatus: "Pilot only", areaValue: 1800, areaUnit: "SQ_FT", cityCountry: "Synthetic Pilot Region", floorCount: 1, locationLink: "https://maps.example.invalid/test-only-founder-pilot", constraints: "No real client or location data." }, needs: { mainChallenge: "Validate the complete Founder Edition workflow.", desiredOutcome: "A deterministic protected one-floor report.", urgency: "PILOT" }, consent: { version: "uchit-intake/v1", contact: true, accuracy: true, confidentiality: true, confirmedAt: PILOT_TIME }, idempotencyKey: "pilot-intake-key", createdAt: PILOT_TIME })],
    ...
    vastuCases: [owned({ id: pilotIds.caseId, caseNumber: "UV-PILOT-CASE-001", clientId: pilotIds.clientId, proposalId: pilotIds.proposalId, projectId: pilotIds.projectId, revisionNumber: 1, serviceType: "EXISTING_SPACE", serviceTemplateVersion: "founder-existing/v1", scopeVersion: "pilot-scope/v1", canonicalStage: "REPORTING", status: "REPORT_APPROVAL_PENDING", reportStatus: "APPROVED", orientationLocked: true, balanceApproved: true, fullPaymentApproved: true, stageAVerdictStatus: "PRESENTED", stageAVerdictPresentedAt: PILOT_TIME, stageAVerdictPresentedByActorUserId: pilotIds.founderId, inputReadiness: { floorPlans: true, siteLocation: true, visualRecord: true, currentUse: true, clientPriorities: true }, createdAt: PILOT_TIME })],
    floorWorkspaces: [owned({ id: pilotIds.floorId, projectId: pilotIds.projectId, caseId: pilotIds.caseId, floorLabel: "Ground floor", status: "LOCKED", locked: true, stageAVerdictStatus: "PRESENTED", stageAVerdictPresentedAt: PILOT_TIME, stageAVerdictPresentedByActorUserId: pilotIds.founderId, reportStatus: "APPROVED", evidenceUploads: [], createdAt: PILOT_TIME })],
    ...
    caseDocuments: [...pilotCaseDocuments, owned({ id: pilotIds.manualSheetId, caseId: pilotIds.caseId, caseRevisionNumber: 1, serviceType: "EXISTING_SPACE", floorLabel: "Ground floor", assetType: "MANUAL_UTILITY_SHEET", versionLabel: "Synthetic manual utility sheet v1", documentDate: "2026-08-11", isCurrent: true, evidenceRef: "case-file-pilot-manual-sheet", blocker: false, ownerRole: "CONSULTANT", ownerName: "Founder Pilot Operator", revisionStatus: "VERIFIED", founderApprovalStatus: "APPROVED", founderApprovedAt: PILOT_TIME, founderApprovedByActorUserId: pilotIds.founderId, idempotencyKey: "pilot-manual-sheet", version: 1, received: { actorId: pilotIds.founderId, actorName: "Founder Pilot Operator", actorRole: "SUPER_ADMIN", at: PILOT_TIME }, verified: { actorId: pilotIds.founderId, actorName: "Founder Pilot Operator", actorRole: "SUPER_ADMIN", at: PILOT_TIME }, updated: { actorId: pilotIds.founderId, actorName: "Founder Pilot Operator", actorRole: "SUPER_ADMIN", at: PILOT_TIME }, createdAt: PILOT_TIME })],
