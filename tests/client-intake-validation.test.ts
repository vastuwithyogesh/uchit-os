...
import { getClientIntakeCompleteness, validateClientIntake } from "../lib/client-intake.ts";
...
test("each required field, URL and coordinate pair receives an exact validation error", () => {
  assert.match(validateClientIntake({ ...valid, challenge: "" }).challenge!, /main challenge/i);
