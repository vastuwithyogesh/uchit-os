...
- `/evaluation` is retained as a technical fallback under More, but its dense console and chart tools are collapsed behind explicit disclosures; its primary recovery returns to `/founder/continue`.
- `/payment-proofs` is retained as a technical fallback under More, with its legacy receipt uploader collapsed behind an explicit disclosure; its primary recovery returns to the Founder balance step (`/founder/12`).
- `/reports` is retained as a technical fallback under More, with the legacy report console collapsed behind an explicit disclosure; its primary recovery returns to Founder report assembly (`/founder/15`).
...
The next QA slice also simplifies the legacy `/reports` surface: report status, preview/approval summaries and archive history are progressive disclosures, and its primary “Open active case” path now goes to `/clients-cases` rather than the legacy `/ops` console. The report action contracts remain intact; hosted read-only inspection reached the expected blocked/no-case state without any mutation. A clean synthetic authenticated browser capture remains the owner-review dependency because the available staging state contains pre-existing records.

Final legacy-path check removed the remaining Evaluation blocker CTA to `/ops`: incomplete evaluation readiness now recovers through `/clients-cases`, preserving the same server-derived prerequisite list and disabled evaluation actions. This closes the last identified primary-journey dependency on the legacy console in the inspected surfaces.
