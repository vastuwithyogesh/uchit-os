...
`;function ne(e,t){return{...e,...t,clients:t.clients?.length?t.clients:e.clients,leadQualifications:t.leadQualifications?.length?t.leadQualifications:e.leadQualifications,commercialProposals:t.commercialProposals?.length?t.commercialProposals:e.commercialProposals,reviewCallBookings:t.reviewCallBookings?.length?t.reviewCallBookings:e.reviewCallBookings,payments:t.payments?.length?t.payments:e.payments,advanceVerifications:t.advanceVerifications?.length?t.advanceVerifications:e.advanceVerifications,vastuCases:t.vastuCases?.length?t.vastuCases:e.vastuCases,floorWorkspaces:t.floorWorkspaces?.length?t.floorWorkspaces:e.floorWorkspaces,reportVersions:t.reportVersions?.length?t.reportVersions:e.reportVersions,evaluationSnapshots:t.evaluationSnapshots?.length?t.evaluationSnapshots:e.evaluationSnapshots,mapping32D:t.mapping32D?.length?t.mapping32D:e.mapping32D,mapping16D:t.mapping16D?.length?t.mapping16D:e.mapping16D,utilityRules:t.utilityRules?.length?t.utilityRules:e.utilityRules,shaktiSnapshots:t.shaktiSnapshots?.length?t.shaktiSnapshots:e.shaktiSnapshots,timelineEvents:t.timelineEvents?.length?t.timelineEvents:e.timelineEvents,optInLeads:t.optInLeads?.length?t.optInLeads:e.optInLeads,whatsappTemplates:t.whatsappTemplates?.length?t.whatsappTemplates:e.whatsappTemplates,whatsappLogs:t.whatsappLogs?.length?t.whatsappLogs:e.whatsappLogs}}async function re(){let t=e();if(!t.DB)return null;await t.DB.prepare($).run();let n=await t.DB.prepare(`SELECT payload FROM app_state_snapshot WHERE id = ?`).bind(`current`).first();return n?.payload?JSON.parse(n.payload):null}async function ie(t){let n=e();return n.DB?(await n.DB.prepare($).run(),await n.DB.prepare(`DELETE FROM app_state_snapshot WHERE id = ?`).bind(`current`).run(),await n.DB.prepare(`INSERT INTO app_state_snapshot (id, payload, updated_at) VALUES (?, ?, ?)`).bind(`current`,JSON.stringify(t),new Date().toISOString()).run(),t):null}async function ae(){let e=C(),t=await re();if(t){let n=ne(e,t);return w(n),n}let n=e;return n.optInLeads=await L(),n.reviewCallBookings=await W(),n.advanceVerifications=await Z(),n}async function oe(e=C()){let t=structuredClone(e);return w(t),await ie(t),await Promise.all([R(t.optInLeads),G(t.reviewCallBookings),Q(t.advanceVerifications)]),t}export{L as a,j as c,M as d,C as f,G as i,D as l,w as m,oe as n,R as o,T as p,Q as r,A as s,ae as t,O as u};