...
export function MethodologyConsole() {
  ...
  const canPublish = Boolean(draft && rules.length > 0 && fixtures.length > 0 && unresolvedRules === 0 && unresolvedFixtures === 0 && module !== "STAGE_B_REMEDIAL");
  ...
  return <section className="section-grid" aria-labelledby="methodology-title">
    ...
    <div className="card span-12"><div className="eyebrow">Step 4</div><h2>{module === "STAGE_B_REMEDIAL" ? "Remedial publication is blocked" : "Publish an immutable methodology version"}</h2><p className="subtle">Publication requires every rule and golden fixture to be Approved. The active predecessor is retired, never rewritten.</p><div className="field"><label htmlFor="publish-reason">Approval reason</label><textarea id="publish-reason" value={reason} onChange={(event) => setReason(event.target.value)} minLength={20} maxLength={500} /></div><button className="button" type="button" disabled={busy || !canPublish || reason.trim().length < 20} onClick={() => { if (window.confirm("Publish this immutable methodology version? Later changes require a new version.")) void run("methodology-version-publish", { methodologyVersionId: draft.id, reason }, draft.recordVersion ?? 0, "Methodology version published and locked."); }}>Publish approved version</button>{module === "STAGE_B_REMEDIAL" && <p role="alert">Blocked — Methodology Input Required. Yogesh’s remedial PRD and approved logic are required before publication.</p>}</div></>}
