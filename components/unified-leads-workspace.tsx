...
export function UnifiedLeadsWorkspace() {
  ...
  const allowedTargets = pipeline ? getAllowedPipelineTransitions(pipeline.stage) : [];
  ...
  return (
    <section className="unified-leads-workspace" aria-labelledby="unified-leads-title">
      <div className="unified-leads-header">
        <div>
          <div className="eyebrow">Canonical lead workspace</div>
          <h2 id="unified-leads-title">Leads and pipeline</h2>
          <p className="subtle">One Uchit view for native opt-ins and future Lovable-origin records. Uchit owns the current stage, owner and next action.</p>
        </div>
      ...
      <div className="unified-leads-controls" aria-label="Lead filters">
        ...
        <label className="field"><span>Stage</span><select value={stageFilter} onChange={(event) => setStageFilter(event.target.value)} aria-label="Filter by pipeline stage"><option value="ALL">All stages</option>{canonicalPipelineStages.map((stage) => <option key={stage} value={stage}>{stageLabel(stage)}</option>)}</select></label>
        ...
        <div className="unified-leads-view-toggle" aria-label="Lead view"><button type="button" className={view === "LIST" ? "button" : "button-secondary"} onClick={() => setView("LIST")} aria-pressed={view === "LIST"}>List</button><button type="button" className={view === "BOARD" ? "button" : "button-secondary"} onClick={() => setView("BOARD")} aria-pressed={view === "BOARD"}>Stages</button></div>
      </div>
      ...
      {!hasRows ? <div className="workspace-state" role="status"><h3>No leads yet</h3><p>Native opt-ins will appear here. Lovable-origin leads remain unavailable until the signed connector is explicitly activated.</p><a className="button" href="/founder/01">Start client readiness</a></div> : filteredEmpty ? <div className="workspace-state" role="status"><h3>No matching leads</h3><p>Clear a filter or search term to see the canonical list.</p><button type="button" className="button-secondary" onClick={() => { setQuery(""); setSourceFilter("ALL"); setStageFilter("ALL"); }}>Clear filters</button></div> : (
        <div className={`unified-leads-layout view-${view.toLowerCase()}`}>
          <div className="unified-leads-list" aria-label={view === "LIST" ? "Lead list" : "Lead stage board"}>
            {view === "LIST" ? visibleRows.map((row) => <button type="button" className={`unified-lead-row ${selected?.id === row.id ? "is-selected" : ""}`} key={row.id} onClick={() => setSelectedId(row.id)} aria-pressed={selected?.id === row.id}>
          ...
          <aside className="unified-lead-detail" aria-labelledby="lead-detail-title">
            {selected ? <>
              ...
              <dl className="unified-lead-fields"><div><dt>Contact</dt><dd>{selected.email || selected.phone || "Not supplied"}</dd></div><div><dt>Attribution</dt><dd>{selected.source || "Native Uchit"}{selected.city ? ` · ${selected.city}` : ""}</dd></div><div><dt>Next action</dt><dd>{selected.nextAction?.summary ?? "Not set"}</dd></div><div><dt>Due</dt><dd>{readableDate(selected.nextAction?.dueAt)}</dd></div><div><dt>Source history</dt><dd>{selected.sourceRecordId ? "Lovable source history linked" : "Native Uchit record"}</dd></div></dl>
