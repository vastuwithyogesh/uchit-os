...
export function UnifiedLeadsWorkspace({ mode = "all" }: { mode?: UnifiedLeadsWorkspaceMode }) {
  ...
  const [proposalAdvanceReceived, setProposalAdvanceReceived] = useState("");
  const [proposalPaymentMode, setProposalPaymentMode] = useState("BANK_TRANSFER");
  const [proposalPaymentReference, setProposalPaymentReference] = useState("");
  ...
  function openProposalDraft(project: NonNullable<Bootstrap["prospectiveProjects"]>[number]) {
    ...
    setProposalAdvanceReceived(String(Math.round((policy?.referenceAdvancePaise ?? 1_100_000) / 100)));
    setProposalPaymentMode("BANK_TRANSFER");
    setProposalPaymentReference("");
    setProposalReason("");
  }

  async function createInboundOnboarding() {
    ...
    const advanceReceivedPaise = Math.round(Number(proposalAdvanceReceived) * 100);
    if (![professionalFeePaise, appliedGstBasisPoints, agreedAdvancePaise, advanceReceivedPaise].every((value) => Number.isSafeInteger(value) && value >= 0)) { setMessage("Enter valid non-negative commercial amounts before onboarding."); return; }
    if (proposalClassification !== "INTERNAL_COMPLIMENTARY" && (!professionalFeePaise || advanceReceivedPaise < agreedAdvancePaise || !proposalPaymentReference.trim())) { setMessage("Paid onboarding requires the agreed advance received and an internal payment reference."); return; }
    ...
    if ((proposalClassification !== "STANDARD_PAID" || hasTermDeviation) && proposalReason.trim().length < 10) { setMessage("Add a private Founder reason for a discounted or complimentary engagement."); return; }
    ...
    try {
      ...
      if (!latestProject) throw new Error("The prospective project is no longer available. Refresh and retry.");
      const response = await fetch("/api/actions", { method: "POST", headers: buildActionHeaders(activeUser.role), body: JSON.stringify({
        action: "founder-inbound-onboarding-create", clientId: selectedClient.id, prospectiveProjectId: latestProject.id,
        classification: proposalClassification, professionalFeePaise, appliedGstBasisPoints, agreedAdvancePaise,
        advanceReceivedPaise: proposalClassification === "INTERNAL_COMPLIMENTARY" ? 0 : advanceReceivedPaise,
        paymentId: proposalClassification === "INTERNAL_COMPLIMENTARY" ? undefined : proposalPaymentReference.trim(), paymentMode: proposalClassification === "INTERNAL_COMPLIMENTARY" ? undefined : proposalPaymentMode,
        classificationReason: proposalClassification === "STANDARD_PAID" ? undefined : proposalReason.trim(), feeDeviationReason: hasTermDeviation ? proposalReason.trim() : undefined, gstDeviationReason: hasTermDeviation ? proposalReason.trim() : undefined, advanceExceptionReason: hasTermDeviation ? proposalReason.trim() : undefined,
        idempotencyKey: `founder:inbound-onboarding:${latestProject.id}:${crypto.randomUUID()}`, expectedProjectVersion: latestProject.recordVersion ?? 0, expectedRecordVersion: latestProject.recordVersion ?? 0, expectedRevision: latestBootstrap.persistenceRevision ?? state.persistenceRevision ?? null,
      }) });
      const result = await response.json();
      if (!response.ok || result.ok === false) throw new Error(result.error?.message ?? result.error ?? "The Founder onboarding could not be completed.");
      const caseId = result.result?.caseRecord?.id;
      if (!caseId) throw new Error("The server did not return the new Case. Reload and retry.");
      setProposalProject(undefined); setMessage("Proposal agreed and Case started. Opening Case Workspace…");
      window.location.assign(`/founder/continue?caseId=${encodeURIComponent(caseId)}`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "The Founder onboarding could not be completed."); }
  ...
  async function prepareCommunication(channel: "WHATSAPP" | "EMAIL", values: Record<string, string>, idempotencyKey: string, context?: CommunicationContext) {
    ...
    const response = await fetch("/api/actions", { method: "POST", headers: buildActionHeaders(activeUser.role), body: JSON.stringify({
      ...
      prospectiveProjectIds: communication?.prospectiveProjectId ? [communication.prospectiveProjectId] : undefined,
  ...
  return <><section className={`lead-workspace ${isPipelinePage ? "lead-workspace-pipeline" : "lead-workspace-table"}`} aria-labelledby="unified-leads-title">
    ...
    {proposalProject ? <div className="lead-drawer-layer"><button className="lead-drawer-backdrop" type="button" onClick={() => setProposalProject(undefined)} aria-label="Close onboarding" /><section className="lead-move-sheet" role="dialog" aria-modal="true" aria-labelledby="proposal-draft-title"><span className="eyebrow">Converted lead · Founder commercial</span><h2 id="proposal-draft-title">Converted — ready to onboard</h2><p>Set the mutually agreed terms from the Review call, record the advance received, and start the Case. No client acceptance or payment proof is required for this Founder-led inbound path.</p><label>Commercial type<select value={proposalClassification} onChange={(event) => { const value = event.target.value as typeof proposalClassification; setProposalClassification(value); if (value === "INTERNAL_COMPLIMENTARY") { setProposalFee("0"); setProposalGst("0"); setProposalAdvance("0"); setProposalAdvanceReceived("0"); setProposalPaymentReference(""); } }}><option value="STANDARD_PAID">PAID</option><option value="SPECIAL_DISCOUNTED">PAID (Founder exception)</option><option value="INTERNAL_COMPLIMENTARY">INTERNAL_COMPLIMENTARY</option></select></label><label>Professional fee (₹)<input inputMode="decimal" value={proposalFee} onChange={(event) => setProposalFee(event.target.value)} /></label><label>GST (%)<input inputMode="decimal" value={proposalGst} onChange={(event) => setProposalGst(event.target.value)} /></label><label>Advance agreed / required (₹)<input inputMode="decimal" value={proposalAdvance} onChange={(event) => setProposalAdvance(event.target.value)} /></label>{proposalClassification !== "INTERNAL_COMPLIMENTARY" ? <><label>Advance received (₹)<input inputMode="decimal" value={proposalAdvanceReceived} onChange={(event) => setProposalAdvanceReceived(event.target.value)} /></label><label>Payment mode<select value={proposalPaymentMode} onChange={(event) => setProposalPaymentMode(event.target.value)}><option value="BANK_TRANSFER">Bank transfer</option><option value="CASH">Cash</option><option value="UPI">UPI</option><option value="OTHER">Other internal reference</option></select></label><label>Internal payment reference<input value={proposalPaymentReference} onChange={(event) => setProposalPaymentReference(event.target.value)} placeholder="Founder receipt reference" /></label></> : <p className="meta">Internal complimentary: ₹0 terms, no payment, no proof and no fake transaction.</p>}{proposalNeedsReason ? <label>Private Founder reason<textarea value={proposalReason} onChange={(event) => setProposalReason(event.target.value)} required maxLength={1200} /></label> : null}<div className="lead-move-actions"><button type="button" className="button" disabled={proposalBusy} onClick={() => void createInboundOnboarding()}>{proposalBusy ? "Starting Case…" : "Create proposal & start Case"}</button><button type="button" className="button-secondary" disabled={proposalBusy} onClick={() => setProposalProject(undefined)}>Cancel</button></div></section></div> : null}
