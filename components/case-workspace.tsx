...
export function CaseWorkspace({ items }: { items: CaseWorkspaceItem[] }) {
  ...
  return (
    <section className="workspace" aria-labelledby="workspace-title">
      ...
      {items.length === 0 ? <div className="workspace-state"><h2>You are all caught up</h2><p>No client tasks are assigned to you. Use “Add or find a client” in the CRM when new work arrives.</p><Link prefetch={false} className="button-secondary" href="/crm">Add or find a client</Link></div> : null}
        ...
        {filtered.map((item) => (
          <article className="workspace-item" key={`${item.clientId}:${item.caseId ?? "lead"}`}>
            ...
            <div className="workspace-actions" aria-label={`Actions for ${item.clientName}`}>
              {item.links.map((link, index) => <Link prefetch={false} className={index === 0 ? "button" : "button-secondary"} href={link.href} key={link.href}>{index === 0 ? `Do this: ${item.nextAction}` : link.label}</Link>)}
            </div>
            <details>
              ...
              {item.caseNumber?.match(/-R\d+$/) ? <p className="meta">This is a rectification revision linked to predecessor case {item.caseNumber.replace(/-R\d+$/, "")}. The earlier report remains unchanged. <Link prefetch={false} href="/timeline">View revision history</Link>.</p> : null}
            </details>
          </article>
        ))}
      </div>
