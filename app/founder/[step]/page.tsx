  ...
  try {
    ...
    if (context.caseId) {
      ...
      if (!candidate || !canAccessFounderCase(state, access.actor, candidate)) {
        return <main className="page-shell"><SiteHeader title="Founder workflow" subtitle="Focused module" minimal /><section className="workspace-state" role="alert"><h1>CASE_NOT_ACCESSIBLE</h1><p>The requested case does not exist or is outside your organisation. Nothing was changed.</p><a className="button-secondary" href="/">Back to scorecard</a></section></main>;
      }
      if (context.floorId) {
        const floor = state.floorWorkspaces.find((item) => item.id === context.floorId);
        if (!floor) return <main className="page-shell"><SiteHeader title="Founder workflow" subtitle="Focused module" minimal /><section className="workspace-state" role="alert"><h1>FLOOR_NOT_ACCESSIBLE</h1><p>The requested floor does not exist or is outside your organisation. Nothing was changed.</p><a className="button-secondary" href={`/founder/${step}?caseId=${encodeURIComponent(context.caseId)}`}>Return to case</a></section></main>;
        if (floor.caseId !== candidate.id) return <main className="page-shell"><SiteHeader title="Founder workflow" subtitle="Focused module" minimal /><section className="workspace-state" role="alert"><h1>FLOOR_CASE_MISMATCH</h1><p>The requested floor does not belong to this case. Nothing was changed.</p><a className="button-secondary" href={`/founder/${step}?caseId=${encodeURIComponent(context.caseId)}`}>Return to case</a></section></main>;
      }
    }
