...
export default async function FounderCommercialProposalStepPage({ params }: { params: Promise<{ proposalId: string; step: string }> }) {
  ...
  const advance = snapshot.state.founderCommercialPaymentConfirmations.find((item) => item.organisationId === access.actor.organisationId && item.proposalVersionId === proposal.id && item.type === "ADVANCE");
  const caseRecord = snapshot.state.vastuCases.find((item) => item.organisationId === access.actor.organisationId && item.proposalId === proposal.id && item.clientId === proposal.clientId);
  return <main className="page-shell"><SiteHeader title="Commercial proposal" subtitle="One section at a time" minimal /><FounderCommercialProposalEditor proposal={proposal} step={Number(step)} revision={snapshot.revision ?? 0} blockers={getFounderProposalBlockers(snapshot.state, proposal)} hasArtifact={snapshot.state.founderProposalArtifacts.some((item) => item.proposalVersionId === proposal.id)} activeNoRefundPolicy={activeNoRefundPolicy ? { title: activeNoRefundPolicy.title, version: activeNoRefundPolicy.version, exactText: activeNoRefundPolicy.exactText } : undefined} paidHandoff={{ advanceConfirmed: Boolean(advance), advanceAmountPaise: advance?.amountPaise, caseId: caseRecord?.id, caseNumber: caseRecord?.caseNumber }} />{readiness ? <FounderStatutoryReadinessCard status={readiness.status} blockers={readiness.blockers} documents={readiness.documents} revision={snapshot.revision ?? 0} /> : null}</main>;
}
