...
async function selectedScope(organisationId: string, caseId: string, floorLabel?: string, localFixture = false) {
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, assets: await listCaseFiles(await selectedScope(organisationId, caseId, floorLabel, isExplicitLocalDemo(request.headers))) }, { headers: { "Cache-Control": "private, no-store", "X-Content-Type-Options": "nosniff" } });
...
export async function POST(request: Request) {
  try {
    ...
    const asset = await saveCaseFileUpload(file, await selectedScope(organisationId, caseId, floorLabel, isExplicitLocalDemo(request.headers)), access.actor);
