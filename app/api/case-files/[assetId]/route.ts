...
export async function GET(request: Request, { params }: { params: Promise<{ assetId: string }> }) {
  ...
  const localFixture = isExplicitLocalDemo(request.headers);
  const state = await loadStateFromPersistence(); const caseRecord = state.vastuCases.find((item) => item.id === caseId);
  const scopedOrganisationId = localFixture ? (caseRecord?.organisationId ?? organisationId) : organisationId;
  if (!caseRecord || (!localFixture && caseRecord.organisationId && caseRecord.organisationId !== organisationId) || getActiveCaseForClient(state, caseRecord.clientId)?.id !== caseId) return NextResponse.json({ ok: false, error: "Active case not found." }, { status: 404 });
  if (floorLabel && !state.floorWorkspaces.some((item) => item.caseId === caseId && (localFixture || !item.organisationId || item.organisationId === scopedOrganisationId) && item.floorLabel === floorLabel)) return NextResponse.json({ ok: false, error: "Case file not found in this floor." }, { status: 404 });
  const { assetId } = await params;
  const file = await readCaseFile(assetId, { organisationId: scopedOrganisationId, caseId, caseRevisionNumber: caseRecord.revisionNumber ?? 1, serviceType: normalizeCaseService(caseRecord).serviceType, floorLabel });
