...
export async function GET(request: Request, context: { params: Promise<{ reportId: string }> }) {
  ...
  if (report.artifact?.templateVersion === "uchit-verdict/v3" || report.artifact?.templateVersion === "uchit-verdict/v4" || report.artifact?.templateVersion === "uchit-verdict/v5") return new Response("Founder Edition v3/v4/v5 reports are available only through the protected PDF export and print route.", { status: 409, headers: { "cache-control": "private, no-store", "x-content-type-options": "nosniff" } });
